...
\label{polar_time}
\end{equation}
Equation~\ref{polar_time} has two time-dependent parts. The first is the sinusoidal term, oscillating at frequency $\sqrt{\omega_\textrm{sp}^2-\gamma^2/4}$. The second is the exponential term, decaying with width $\gamma/2$. These two terms show that a plasmon has time dynamics consistent with a damped harmonic oscillator. This is to be expected, as the polarizability is related to the harmonic oscillator Green's function through $\textrm{Im}\{g(\omega)\} = (4\pi/a^3\omega_{\textrm{sp}}^2)\textrm{Im}\{\alpha(\omega)\}$\cite{ARPC,Quillin}. Using Drude model parameters fit from experimental data\cite{JC,Rakic,Weaver,Segall} allows us to compute the time-dependent response of a dipole plasmon for different metals, as shown in Fig. \ref{alpha_metals} for silver, gold, platinum, and aluminum. Eq. \ref{polar_time} and Fig. \ref{alpha_metals} show the dependence of the plasmon oscillator strength, frequency, and lifetime on material properties.
