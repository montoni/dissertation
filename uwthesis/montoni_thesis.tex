...
\end{equation}
Defining the Green's function $e^{\textrm{i}k|\textbf{x}-\textbf{x}'}/|\textbf{x}-\textbf{x}'| = 4\pi G(\textbf{x},\textbf{x}')$ allows us to write down a pseudo-potential $\psi = \int dz e^{-\textrm{i}qz} G(\textbf{x},\textbf{x}')$. Integrating by parts, we can move the gradient in Eq. \ref{}

\begin{equation}
\int dz e^{-\textrm{i}qz} \nabla G(\textbf{x},\textbf) = e^{-\textrm{i}qz} G(\textbf{x},\textbf{x}') \Big|_{-\infty}^{\infty} + \textrm{i}q \psi
\label{psi_integral}
\end{equation}

We can actually rewrite $\psi$ in terms of the Fourier transform of $G$, which will help us take the integral in $z$.
...
So, let's rewrite the EELS rate just so we know where we are.
...
Again, the ultimate goal here is to get the EELS rate in terms of the electron electric field and the induced dipole moments. Well, we can rewrite the induced charge density and current density in terms of induced dipole moments like so
...
Let's put these definitions into the EELS rate.
...
Recognizing that $\psi\textbf{d}\cdot\nabla_r\delta(\textbf{x}'-\textbf{r}) = \textbf{d}\cdot(\nabla_r\psi)\delta(\textbf{x}'-\textbf{r})$, we can evaluate the volume integral resulting in $\textbf{x}' \rightarrow \textbf{r}$. Also, we can evaluate $\nabla_r\psi(\textbf{x}_{\perp},\textbf{r},\omega) = -\textrm{i}\textbf{k}'\psi(\textbf{x}_{\perp},\textbf{r},\omega)$. The EELS rate becomes
...
Let's notice a few more things. Since the electron has momentum only in the z-direction, $q\hat{\textbf{z}} = \textbf{q}$. Let's also write out $\psi$ in full:
